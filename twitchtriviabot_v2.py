...
class TriviaBot(object):
    ...
    def validate_trivia_config(self,temp_config):
        for k, v in temp_config.items():
            if k == 'filename':
                if not v.endswith(".csv") or not v.endswith(".xlsx") or not v.endswith(".xls"):
                    self.error_msg = "Config error: Filename %s does not end with .csv, .xlsx or .xls " % v
...
class Session(object):
    def __init__(self, cb, trivia_config):
        ...
        try:
            ...
            if ".xls" in self.session_config['file_name'] or ".xlsx" in self.session_config['file_name']:
                self.ts = pd.read_excel(os.path.join('config',self.session_config['file_name']),dtype='str')
            else:
                self.ts = pd.read_csv(os.path.join('config',self.session_config['file_name']),dtype='str')
                
            self.ts.fillna('',inplace=True)
            self.ts = self.ts[self.ts['answer'] != '']
            self.tsrows = self.ts.shape[0]
            
            if self.session_config['mode'] == 'poll2':
                logging.debug("Mode set to poll2, only taking questions with valid answer/answer2...")
                self.ts = self.ts[self.ts['answer2'] != '']


            logging.debug("Setting up session set...")
            ...
            try:
                self.ss = self.ts.sample(self.session_config['question_count'])
            except:
                logging.debug("Forcing session set to be the same set as entire trivia set (all questions will be asked)")
                self.ss = self.ts.copy()
            try:

                for i, r in self.ss.iterrows():
                    self.questions.append(Question(r, self.session_config))
